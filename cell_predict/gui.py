...
class HelloFrame(wx.Frame):
    ...
    def __init__(self,*args,**kw):
        ...
        self.pnl = pnl
        # st = wx.StaticText(pnl, label="白细胞识别", pos=(100, 0))
        # font = st.GetFont()
        # font.PointSize += 10
        # font = font.Bold()
        # st.SetFont(font)
        ...
        self.SetStatusText("Welcome to cell world")
    ...
    def OnSelect(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            img = Image.open(dialog.GetPath()).convert("RGB")
            imag = img.resize([64, 64])
            image = np.array(imag)
            result = evaluate_one_image(image)
            result_text = wx.StaticText(self.pnl, label=result, pos=(170, 20))
...
if __name__ == '__main__':
    ...
    app = wx.App()
    frm = HelloFrame(None, title='白细胞识别系统', size=(700,500))
    frm.Show()
    app.MainLoop()